...
/** Format mã HTML theo chiều dọc sạch sẽ, dễ đọc khi xem dạng mã nguồn (Code View) */
...
export const RichTextEditor = React.memo(
  forwardRef<RichTextEditorRef, RichTextEditorProps>(
    ({ value, onChange, placeholder = 'Nhập nội dung email...', onFocus, onBlur, error, minHeight = '240px' }, ref) => {
      ...
      // Đồng bộ value từ ngoài vào visual contentEditable
      useEffect(() => {
        if (visualEditorRef.current && mode === 'visual') {
          // Nếu người dùng đang tương tác/focus trực tiếp trong editor thì không can thiệp vào DOM
          const isUserFocusing =
            document.activeElement === visualEditorRef.current ||
            visualEditorRef.current.contains(document.activeElement);
          ...
          // Nếu value thay đổi từ bên ngoài (reset form, chọn template khác...)
          if (value !== lastHtmlRef.current || currentHtml !== (value || '')) {
            lastHtmlRef.current = value || '';
      ...
      return (
        <div className="flex flex-col rounded-md border border-neutral-300 bg-white overflow-hidden shadow-2xs focus-within:border-primary-500 focus-within:ring-2 focus-within:ring-primary-100 transition">
          ...
          <div className="flex flex-wrap items-center justify-between gap-1 border-b border-neutral-200 bg-neutral-100 px-2 py-1.5 text-neutral-700 select-none">
            ...
            {/* Nút Switch chuẩn: 1 biểu tượng </> duy nhất */}
            <div className="flex items-center">
              <button
                type="button"
                onClick={toggleMode}
                className={`flex items-center justify-center rounded px-2.5 py-1 text-xs font-mono font-bold transition shadow-2xs ${mode === 'code'
          ...
          <div className="relative flex-1 bg-white">
            {mode === 'visual' ? (
              ...
              /* HTML Source Code Editor (Dark Mode theo chiều dọc chuẩn email, tự ngắt dòng) */
              <div className="relative flex bg-[#1e1e1e] text-emerald-400 font-mono text-sm">
                <textarea
                  ...
                  onFocus={onFocus}
                  onBlur={onBlur}
                  className="w-full resize-y bg-transparent p-4 text-emerald-300 placeholder-neutral-600 focus:outline-none font-mono leading-relaxed selection:bg-neutral-700 whitespace-pre-wrap break-words"
                  style={{ minHeight }}
